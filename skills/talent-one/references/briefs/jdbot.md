...
Source fingerprint: b133c32d0dc5
Inputs: 00_SHARED_CORE.md, 02_GROUNDING_MATRIX.md, CONTEXT_PACK.md, content-schemas/TK_02.content-schema.json
...
Your bound content schema: `TK_02.content-schema.json`. This brief's SCHEMA SUMMARY section below is the compact form (slot names, types, word ranges, repeat/toggle/optional structure); it replaces reading the schema JSON and the worked example-content JSON directly.
...
- Required: role, spec.requirements, spec.must_have_skills, comp.canonical_range, keywords.title_synonyms
- Optional but used: spec.nice_to_have, constraints, spec.assessment_methods
...
*(Fidelity override: `market.classification` added beyond the §5 row above because jdbot.md's own METHOD section reads it; see F2 run report for the source-file gap this covers.)*
...
## SCHEMA SUMMARY · `TK_02.content-schema.json` (template: `TK_02_Job_Description.dc.html`)
...
- cover.compliance (prose_short 6-6)
- s1.title (label 2-7)
- s1.body (prose_long 18-85)
- jd.title (label 3-3)
- jd.meta (prose_short 9-9)
- jd.intro (prose_long 80-80)
- jd.close (prose_long 31-31)
- jd.benefits (label ?-?)
- jd.eeo (label ?-?)
- s2.title (label 2-7)
...
- jd.responsibilities [rows n=3-6]
  - resp.item (prose_short 18-18)
- jd.requirements [rows n=3-6]
  - req.item (prose_short 17-17)
- annotations [rows n=3-6]
  - note.tag (label 1-3)
  - note.body (prose_short 11-26)
- ledger.rows [rows n=3-8]
  - ledger.before (prose_short 5-5)
  - ledger.after (prose_short 5-5)
  - ledger.effect (delta 3-3)
- comp.rows [rows n=2-4 bars=1(range)]
  - comp.label (label 2-4)
  - comp.n (prose_short 6-7)
  - comp.low (currency 1-1)
  - comp.high (currency 1-1)

### Optional blocks
- `optional_1` (handoff_block=True): Handoff → next artifact Internal · optional: delete before sharing outside the search team Producesm
